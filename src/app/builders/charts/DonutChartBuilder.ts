...
export class DonutChartBuilder extends BaseChartBuilder {
  build(config: IChartConfig): string {
    const { data, labels = (data as number[])?.map((_, i) => `Category ${i + 1}`) ?? [], title = 'Donut Chart', subtitle = '', width = 600, height = 400 } = config;
    ...
    try {
      ...
      Highcharts.chart(container, {
        ...
        plotOptions: { series: { animation: false }, pie: { innerSize: '60%', dataLabels: { enabled: true, format: '{point.name}: {point.percentage:.1f}%', style: { fontSize: '11px', fontWeight: '500', color: '#374151', textOutline: 'none' } } } },
