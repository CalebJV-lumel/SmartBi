...
export class BubbleChartBuilder extends BaseChartBuilder {
  build(config: IChartConfig): string {
    const { data, title = 'Bubble Chart', subtitle = '', width = 600, height = 400 } = config;
    const container = this.createContainer(width, height);

    try {
      Highcharts.chart(container, {
        chart: { type: 'bubble', width, height, backgroundColor: '#ffffff', animation: false, style: { fontFamily: 'Inter, Arial, sans-serif' } },
        ...
        plotOptions: { series: { animation: false }, bubble: { dataLabels: { enabled: true, style: { fontSize: '10px', fontWeight: '500', color: '#374151', textOutline: 'none' } } } },
        series: [{ type: 'bubble', name: 'Data', data: data as number[][], color: '#667eea' }],
