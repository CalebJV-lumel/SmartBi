...
export class BoxPlotBuilder extends BaseChartBuilder {
  build(config: IChartConfig): string {
    const { data, labels = (data as number[][])?.map((_, i) => `Group ${i + 1}`) ?? [], title = 'Box Plot', subtitle = '', width = 600, height = 400 } = config;
    const container = this.createContainer(width, height);

    try {
      Highcharts.chart(container, {
        chart: { type: 'boxplot', width, height, backgroundColor: '#ffffff', animation: false, style: { fontFamily: 'Inter, Arial, sans-serif' } },
        ...
        series: [{ type: 'boxplot', name: 'Distribution', data: data as number[][], color: '#667eea' }],
