...
export class LineChartBuilder extends BaseChartBuilder {
  build(config: IChartConfig): string {
    const { data, labels = (data as number[])?.map((_, i) => `Point ${i + 1}`) ?? [], title = 'Line Chart', subtitle = '', width = 600, height = 400 } = config;
    const container = this.createContainer(width, height);

    try {
      Highcharts.chart(container, {
        chart: { type: 'line', width, height, backgroundColor: '#ffffff', animation: false, style: { fontFamily: 'Inter, Arial, sans-serif' } },
        ...
        plotOptions: { series: { animation: false }, line: { marker: { enabled: true, radius: 4 }, dataLabels: { enabled: true, style: { fontSize: '11px', fontWeight: '500', color: '#374151', textOutline: 'none' } } } },
        series: [{ type: 'line', name: 'Value', data: data as number[], color: '#667eea' }],
