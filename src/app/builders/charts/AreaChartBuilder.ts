...
export class AreaChartBuilder extends BaseChartBuilder {
  build(config: IChartConfig): string {
    const { data, labels = (data as number[])?.map((_, i) => `Point ${i + 1}`) ?? [], title = 'Area Chart', subtitle = '', width = 600, height = 400 } = config;
    const container = this.createContainer(width, height);

    try {
      Highcharts.chart(container, {
        chart: { type: 'area', width, height, backgroundColor: '#ffffff', animation: false, style: { fontFamily: 'Inter, Arial, sans-serif' } },
        ...
        plotOptions: { series: { animation: false }, area: { fillOpacity: 0.5, marker: { enabled: false }, dataLabels: { enabled: true, style: { fontSize: '11px', fontWeight: '500', color: '#374151', textOutline: 'none' } } } },
        series: [{ type: 'area', name: 'Value', data: data as number[], color: '#667eea' }],
